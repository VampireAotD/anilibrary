...
<template>
  ...
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Anime</h2>
    </template>
          ...
          >
            <template #item-title="item: AnimeWithRelations">
              <div class="flex items-center gap-x-3">
                <img class="object-cover w-10 h-10 rounded-full" :src="item.image.path" :alt="item.title"/>

                <Link :href="route('anime.show', item.id)" target="_blank" class="w-[300px]">
                  <p
