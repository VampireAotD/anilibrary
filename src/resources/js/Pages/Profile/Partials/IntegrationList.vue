...
<template>
  <section>
    <header>
      <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Integrations</h2>
    ...
    <div class="mt-4 flex items-center gap-4">
      <div class="p-2 rounded-lg">
        <h3 class="mb-2 text-md font-medium text-gray-900 dark:text-gray-100">Telegram</h3>

        <TelegramLoginWidget v-if="!telegramUser" :callback-handler="handleTelegramLogin"/>

        <div class="flex flex-col" v-else>
          <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Connected as {{ telegramUser.username ?? telegramUser.telegramId }}
          </p>

          <DangerButton @click="revokeTelegramAccount">Revoke Telegram account</DangerButton>
        </div>
      </div>
    </div>
  </section>
